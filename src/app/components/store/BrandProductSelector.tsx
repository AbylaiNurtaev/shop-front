...
export function BrandProductSelector({ brandProducts, categories, onAddProduct, onClose }: BrandProductSelectorProps) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/50 z-50 flex items-end md:items-center justify-center p-0 md:p-4">
      <div className="bg-white w-full h-full md:max-w-6xl md:h-[90vh] md:rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        {/* Header */}
        <div className="bg-white border-b-2 border-gray-200 px-4 md:px-6 py-4 md:py-5 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-xl md:text-2xl font-bold text-gray-900">Добавить товар</h2>
            <p className="text-xs md:text-sm text-gray-600 mt-1 hidden md:block">Выберите товар из каталога брендов</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className={`flex-1 flex flex-col overflow-hidden ${selectedProduct ? 'hidden md:flex' : ''}`}>
            {/* Search & Filters */}
            <div className="bg-gray-50 border-b border-gray-200 px-4 md:px-6 py-3 md:py-4 flex-shrink-0">
                    ...
                    {filteredProducts.map((product) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold text-base text-gray-900 mb-1">{product.name}</h3>
                            <p className="text-xs font-mono text-gray-500">Арт: {product.sku}</p>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                          <div>
                            ...
                            <span className="ml-1 font-medium text-gray-900">{getCategoryName(product.categoryId)}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Упаковка:</span>
                            <span className="ml-1 font-medium text-gray-900">{product.packageInfo || '—'}</span>
                          </div>
                        </div>
                        <button
                          onClick={() => handleSelectProduct(product)}
                          className="w-full flex items-center justify-center gap-2 h-11 bg-blue-600 text-white text-sm font-semibold rounded-lg active:bg-blue-700 transition-colors"
                        >
          ...
          {selectedProduct && (
            <>
              ...
              />
              {/* Mobile Bottom Sheet */}
              <div className="md:hidden fixed inset-x-0 bottom-0 bg-white border-t-2 border-gray-200 rounded-t-3xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden animate-slideUp z-50">
                <div className="px-5 py-4 bg-white border-b-2 border-gray-200 flex items-center justify-between flex-shrink-0">
                  <div>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-5">
                  ...
                  <div className="bg-white border-2 border-blue-200 rounded-xl p-4">
                    <p className="text-xs font-bold text-blue-700 uppercase tracking-wide mb-3">Выбранный товар</p>
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <p className="font-mono text-xs font-semibold text-gray-900">{selectedProduct.sku}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500 mb-1">Категория</p>
                          <p className="text-xs font-semibold text-gray-900">{getCategoryName(selectedProduct.categoryId)}</p>
                        </div>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500 mb-1">Упаковка</p>
                        <p className="text-xs font-semibold text-gray-900">{selectedProduct.packageInfo || '—'}</p>
                      </div>
                    </div>
                  </div>
                ...
                {/* Actions */}
                <div className="px-5 py-4 bg-white border-t-2 border-gray-200 space-y-3 safe-area-inset-bottom">
                  <button
                    onClick={handleConfirmAdd}
                    disabled={initialQuantity === 0 || price === 0 || showSuccess}
                    className="w-full h-12 bg-blue-600 text-white rounded-lg font-bold active:bg-blue-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    onClick={() => setSelectedProduct(null)}
                    className="w-full h-12 border-2 border-gray-300 bg-white text-gray-700 rounded-lg font-bold active:bg-gray-50 transition-colors"
                  >
                    Отмена
                  </button>
                </div>
              ...
              {/* Desktop Right Panel */}
              <div className="hidden md:flex w-96 border-l-2 border-gray-200 bg-gray-50 flex-col overflow-hidden animate-slideInRight">
